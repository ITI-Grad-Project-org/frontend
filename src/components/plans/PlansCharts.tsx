...
interface PlansChartsProps {
  programs: ClientProgramDraft[];
}

export function PlansCharts({ programs }: PlansChartsProps) {
  ...
  const monthData = useMemo(() => {
    const counts = new Map<string, number>();
    for (const program of programs) {
      const key = monthKey(new Date(program.createdAt));
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <CardMain className="min-w-0 overflow-hidden">
        <CardHeader>
          <CardTitle className="text-lg">Plans by status</CardTitle>
          <CardDescription>Distribution of your plans across statuses</CardDescription>
        </CardHeader>
      ...
      <CardMain className="min-w-0 overflow-hidden">
        <CardHeader>
          <CardTitle className="text-lg">Plans created</CardTitle>
